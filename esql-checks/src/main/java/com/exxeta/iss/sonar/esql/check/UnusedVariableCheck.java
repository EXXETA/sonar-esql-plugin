	...
	public void visitProgram(ProgramTree tree) {
		...
		for (String line : lines) {
			...
			for (String Vars : variables) {
				...
				if(thelines.contains(Vars) && !upperCaseTheLine.contains("DECLARE")  && !upperCaseTheLine.contains("REFERENCE") ){
		...
		for (Entry<String, DeclareStatementTree> variable : declaredVariable.entrySet()) {
			...
			if (variable.getValue() != null && Variabl != null){
			addIssue(new PreciseIssue(this, new IssueLocation(variable.getValue(),variable.getValue(), String.format(MESSAGE, variable.getKey()))));
