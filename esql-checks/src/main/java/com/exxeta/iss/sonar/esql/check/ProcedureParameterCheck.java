...
/**
 * This Java Class is created to ensure that all the procedures do not have parameter count above the allowed threshold
 * @author Arjav Shah
 *
 */
@Rule(key = ProcedureParameterCheck.CHECK_KEY, priority = Priority.MAJOR, name = "Function names should comply with a naming convention", tags = Tags.CONVENTION)
public class ProcedureParameterCheck extends DoubleDispatchVisitorCheck {
	public static final String CHECK_KEY = "ProcedureParameter";	
	...
	public void visitCreateProcedureStatement(CreateProcedureStatementTree tree) {
		...
		if (tree.parameterList().size()>maximumParameterThreshold){
			 addIssue(new PreciseIssue(this, new IssueLocation(tree.identifier(), tree.identifier(), "Reduce parameters for procedure \""
